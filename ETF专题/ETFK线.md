# ETFK线（MCP 工具 `ft_etf_candlesticks`）

> **MCP 工具**：`ft_etf_candlesticks`（category: `ETF专题`）。返回统一 MCP 输出：`structuredContent.metadata` + `structuredContent.data`；`content[0].text` 是同值的序列化 JSON，不额外返回 Markdown。输入参数 / 输出参数 / 数据样例见下文。
> 文中 `Response`、`items`、`records`、`code`、`message` 等名称仅为字段说明；MCP 对外固定为上述 `metadata/data`。

- 描述：获取单只 ETF 的历史 K 线，包含开盘价、最高价、最低价、收盘价、成交量和成交额，价格单位为元；支持分、日、周、月、年周期及前复权、后复权，仅适用于 ETF 标的。
- 数据范围：2004 年首只 ETF（华夏上证 50）上市至今
...
- 提示：
  ...
  - `symbol` 必须是 ETF 代码，格式为 `{代码}.{市场}`；沪市支持 `.XSHG`/`.SH`，深市支持 `.XSHE`/`.SZ`。
  - 不带市场后缀、后缀无法识别或传入非 ETF 标的时，当前外部接口返回系统错误。
...
| 名称 | 类型 | 必选 | 描述 |
|------|------|------|------|
| symbol | string | Y | ETF 代码，如 `510300.XSHG`、`159915.XSHE`；也接受 `.SH`、`.SZ` 短后缀 |
...
CALL_RESPONSE=$(curl -fsS -m 60 -X POST "$MCP_BASE_URL" \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_etf_candlesticks","arguments":{"symbol":"510300.SH","interval_unit":"day","until_ts_millis":1782370800000,"limit":2}}}')
    ...
    async with streamable_http_client(MCP_BASE_URL) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            result = await session.call_tool(
                'ft_etf_candlesticks',
                {'symbol': '510300.SH', 'interval_unit': 'day', 'until_ts_millis': 1782370800000, 'limit': 2},
            )
            if result.is_error:
...
|------|------|------|------|------|------|------|------|
| 4.9290 | 4.9780 | 4.9160 | 4.9670 | 1782284400000 | 1782264600000 | 12839317640.6000 | 2590673409 |
| 4.9730 | 5.0550 | 4.9660 | 5.0480 | 1782370800000 | 1782351000000 | 13252886361.4920 | 2640608736 |
| ... | ... | ... | ... | ... | ... | ... | ... |
