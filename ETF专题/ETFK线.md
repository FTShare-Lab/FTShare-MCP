...
> **MCP 工具**：`ft_etf_candlesticks`（category: `ETF专题`）。返回统一 MCP 输出：`structuredContent.metadata` + `structuredContent.data`；`content[0].text` 为同值 JSON。

- 描述：查询单只 ETF 的分钟、日、周、月或年 K 线，仅接受 ETF 标的。
...
| 名称 | 类型 | 必选 | 描述 |
|------|------|------|------|
| symbol | string | Y | ETF 代码，支持 `.XSHG/.XSHE` 和 `.SH/.SZ` |
...
> MCP 工具名 `ft_etf_candlesticks`。MCP Streamable HTTP 要求先 `initialize` 获取 `Mcp-Session-Id`，再发送 `notifications/initialized`，最后调用 `tools/call`。
...
```json
{"open":"4.9730","high":"5.0550","low":"4.9660","close":"5.0480","ts_millis":1782370800000,"ts_millis_open":1782351000000,"turnover":"13252886361.4920","volume":2640608736}
```
