...
curl -sS -m 10 -X POST <MCP_BASE_URL> \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_get_etf_components_handler","arguments":{"symbol": "510300.XSHG"}}}'
```
...
async def main():
    async with streamable_http_client('<MCP_BASE_URL>') as (r, w):
        async with ClientSession(r, w) as s:
            await s.initialize()
            res = await s.call_tool('ft_get_etf_components_handler', {"symbol": "510300.XSHG"})
