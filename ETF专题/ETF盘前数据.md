...
- 描述：获取指定交易日的全部 ETF 盘前信息，包括申赎单位、净值、现金差额、是否公布 IOPV、申赎允许情况和预估现金部分；不传 `date` 时查询中国标准时间当日数据。
- 数据范围：无时间维度（按交易日取），当日 / 指定交易日快照
- 单次限量：无分页，单交易日全部 ETF 盘前信息一次返回
- 提示：
  - 不传 `date` 用当日；传 `date` 用指定交易日（YYYYMMDD）。
...
CALL_RESPONSE=$(curl -fsS -m 60 -X POST "$MCP_BASE_URL" \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_get_etf_pre","arguments":{"date":20260624}}}')
...
| 159003 | 3554 | 1 | 1.0 | 100.0 | 0.0 |
| 159005 | 3554 | 1 | 1.0 | 100.0 | 0.0 |
| ... | ... | ... | ... | ... | ... |
