...
- 描述：根据标的代码查询单只 ETF 在指定交易日的盘前信息，包括申赎单位、净值、现金差额、是否公布 IOPV、申赎允许情况和预估现金部分；不传 `date` 时查询中国标准时间当日数据。
...
CALL_RESPONSE=$(curl -fsS -m 60 -X POST "$MCP_BASE_URL" \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_get_etf_pre_single_handler","arguments":{"symbol":"510300.XSHG","date":20260624}}}')
    ...
    async with streamable_http_client(MCP_BASE_URL) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            result = await session.call_tool(
                ...
                {'symbol': '510300.XSHG', 'date': 20260624},
            )
            if result.is_error:
...
|------|------|------|------|------|------|------|------|
| 510300 | 3553 | 900000 | 0.5 | 300 | 4.947 | 95480.88 | 3 |
| ... | ... | ... | ... | ... | ... | ... | ... |
