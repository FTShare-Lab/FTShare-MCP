...
> **MCP 工具**：`ft_convertible_bond_candlesticks`（category: `债券专题`）。返回统一 MCP 输出：`structuredContent.metadata` + `structuredContent.data`；`content[0].text` 为同值 JSON。

- 描述：查询单只可转债的分钟、日、周、月或年 K 线，仅接受可转债标的。
...
| 名称 | 类型 | 必选 | 描述 |
|------|------|------|------|
| symbol | string | Y | 可转债代码，支持 `.XSHG/.XSHE` 和 `.SH/.SZ` |
...
> MCP 工具名 `ft_convertible_bond_candlesticks`。MCP Streamable HTTP 要求先 `initialize` 获取 `Mcp-Session-Id`，再发送 `notifications/initialized`，最后调用 `tools/call`。
...
```json
{"open":"201.9500","high":"206.5450","low":"194.6660","close":"194.8950","ts_millis":1717570800000,"ts_millis_open":1717551000000,"turnover":"1203839872.3300","volume":5973980}
```
