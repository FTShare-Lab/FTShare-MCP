# 可转债K线（MCP 工具 `ft_convertible_bond_candlesticks`）

> **MCP 工具**：`ft_convertible_bond_candlesticks`（category: `债券专题`）。返回统一 MCP 输出：`structuredContent.metadata` + `structuredContent.data`；`content[0].text` 是同值的序列化 JSON，不额外返回 Markdown。输入参数 / 输出参数 / 数据样例见下文。
> 文中 `Response`、`items`、`records`、`code`、`message` 等名称仅为字段说明；MCP 对外固定为上述 `metadata/data`。

- 描述：获取单只可转债的历史 K 线，包含开盘价、最高价、最低价、收盘价、成交量和成交额，价格单位为元；支持分、日、周、月、年周期及前复权、后复权，仅适用于可转债标的。
- 数据范围：2002 年首只可转债（阳光转债）发行至今
...
- 提示：
  ...
  - `symbol` 必须是可转债代码，格式为 `{代码}.{市场}`；沪市支持 `.XSHG`/`.SH`，深市支持 `.XSHE`/`.SZ`。
  - 不带市场后缀、后缀无法识别或传入非可转债标的时，当前外部接口返回系统错误。
...
| 名称 | 类型 | 必选 | 描述 |
|------|------|------|------|
| symbol | string | Y | 可转债代码，如 `113027.XSHG`、`128048.XSHE`；也接受 `.SH`、`.SZ` 短后缀 |
...
| limit | int | N | 返回条数上限；未传 since 和 limit 时默认最多返回 50 根 K 线 |
...
CALL_RESPONSE=$(curl -fsS -m 60 -X POST "$MCP_BASE_URL" \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_convertible_bond_candlesticks","arguments":{"symbol":"113027.SH","interval_unit":"day","until_ts_millis":1782370800000,"limit":2}}}')
    ...
    async with streamable_http_client(MCP_BASE_URL) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            result = await session.call_tool(
                'ft_convertible_bond_candlesticks',
                {'symbol': '113027.SH', 'interval_unit': 'day', 'until_ts_millis': 1782370800000, 'limit': 2},
            )
            if result.is_error:
...
|------|------|------|------|------|------|------|------|
| 201.9500 | 206.5450 | 194.6660 | 194.8950 | 1717570800000 | 1717551000000 | 1203839872.3300 | 5973980 |
| 208.0000 | 216.6000 | 196.0000 | 203.3990 | 1717657200000 | 1717637400000 | 4078942878.1300 | 19714910 |
| ... | ... | ... | ... | ... | ... | ... | ... |
