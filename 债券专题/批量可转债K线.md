...
> **MCP 工具**：`ft_convertible_bond_candlesticks_batch`（category: `债券专题`）。返回统一 MCP 输出：`structuredContent.metadata` + `structuredContent.data`；`content[0].text` 为同值 JSON。

- 描述：批量查询多只可转债的 K 线，并将嵌套结果展开为带 `symbol` 的逐根记录。
- 提示：所有标的必须是可转债；分钟 K 的起止时间最多覆盖 3 个自然日。

## 输入参数

| 名称 | 类型 | 必选 | 描述 |
|------|------|------|------|
| symbols | array[string] | Y | 可转债代码列表 |
...
> MCP 工具名 `ft_convertible_bond_candlesticks_batch`。MCP Streamable HTTP 要求先 `initialize` 获取 `Mcp-Session-Id`，再发送 `notifications/initialized`，最后调用 `tools/call`。
...
```json
{"symbol":"113027.SH","open":"201.9500","high":"206.5450","low":"194.6660","close":"194.8950","ts_millis":1717570800000,"ts_millis_open":1717551000000,"turnover":"1203839872.3300","volume":5973980}
```
