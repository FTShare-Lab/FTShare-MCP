...
- 描述：按必填的基金代码 `institution_code` 和区间类型 `cal_type` 查询基金累计收益率序列；支持 1M、3M、6M、1Y、3Y、5Y 和 YTD 区间，以复权单位净值计算累计收益率。
- 数据范围：按交易日查询
...
CALL_RESPONSE=$(curl -fsS -m 60 -X POST "$MCP_BASE_URL" \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_get_fund_cal_return","arguments":{"institution_code":"110011","cal-type":"1M"}}}')
    ...
    async with streamable_http_client(MCP_BASE_URL) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            result = await session.call_tool(
                ...
                {'institution_code': '110011', 'cal-type': '1M'},
            )
            if result.is_error:
