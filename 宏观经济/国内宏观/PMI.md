...
> **MCP 工具**：`ft_consumer_pmi_monthly`（category: `宏观经济/国内宏观`）。返回统一 MCP 输出：`structuredContent.metadata` + `structuredContent.data`；`content[0].text` 是同值的序列化 JSON，不额外返回 Markdown。输入参数 / 输出参数 / 数据样例见下文。
> 文中 `Response`、`items`、`records`、`code`、`message` 等名称仅为字段说明；MCP 对外固定为上述 `metadata/data`。

- 描述：查询中国采购经理指数（PMI）月度汇总计算结果，覆盖制造业与非制造业。按月份返回制造业/非制造业 PMI 当月值、同比、环比。数据按月聚合。提示：`month` 格式化为「YYYY年MM月份」；PMI 数值单位为百分比（%），缺失为 null。
...
> MCP 工具名 `ft_consumer_pmi_monthly`。MCP Streamable HTTP 要求**先 initialize 拿 `Mcp-Session-Id`，发送 `notifications/initialized`，再 `tools/call`**，后续请求同时带该 Session ID 和协商后的 `MCP-Protocol-Version`。返回统一 `metadata/data` 结构化输出；`content[0].text` 为同值 JSON 文本，不额外返回 Markdown。
...
async def main():
    async with streamable_http_client('<MCP_BASE_URL>') as (r, w):
        async with ClientSession(r, w) as s:
            await s.initialize()
            res = await s.call_tool('ft_consumer_pmi_monthly', {})
            print(res.structured_content)   # 推荐：统一 metadata/data 结构化输出
