# 批量指数K线（MCP 工具 `ft_index_candlesticks_batch`）

> **MCP 工具**：`ft_index_candlesticks_batch`（category: `指数专题/指数行情`）。返回统一 MCP 输出：`structuredContent.metadata` + `structuredContent.data`；`content[0].text` 是同值的序列化 JSON，不额外返回 Markdown。输入参数 / 输出参数 / 数据样例见下文。
> 文中 `Response`、`items`、`records`、`code`、`message` 等名称仅为字段说明；MCP 对外固定为上述 `metadata/data`。

- 描述：批量获取多个指数的历史 K 线，包含开盘点位、最高点位、最低点位、收盘点位、成交量和成交额，支持分钟、日、周、月、年周期以及前复权、后复权；所有标的使用同一周期，且仅支持指数标的。
- 数据范围：1991 年起；各指数按自身实际发布和可用历史时间返回数据
...
- 提示：
  ...
  - `symbols` 中每项必须是指数代码；沪市支持 `.XSHG`/`.SH`，深市支持 `.XSHE`/`.SZ`。
  - 输入短后缀时，响应中的 symbol 会规范化为 `.XSHG`、`.XSHE` 等长后缀。
  - 若任一 symbol 不是指数，整个批量请求失败，不静默过滤；当前外部接口返回系统错误。
...
| 名称 | 类型 | 必选 | 描述 |
|------|------|------|------|
| symbols | string[] | Y | 指数代码列表，如 `["000300.XSHG","399001.XSHE"]`；也接受 `.SH`、`.SZ` 短后缀 |
...
| 名称 | 类型 | 默认显示 | 描述 |
|------|------|---------|------|
| close | decimal | Y | 收盘点位（或最新点位），单位为指数点；JSON 中为字符串 |
| high | decimal | Y | 最高点位，单位为指数点；JSON 中为字符串 |
| low | decimal | Y | 最低点位，单位为指数点；JSON 中为字符串 |
| open | decimal | Y | 开盘点位，单位为指数点；JSON 中为字符串 |
| symbol | string | Y | 指数代码；响应统一使用 `.XSHG`、`.XSHE` 等长市场后缀 |
| ts_millis | int(ms) | Y | 收盘时间戳，单位毫秒 |
| ts_millis_open | int(ms) | Y | 开盘时间戳，单位毫秒 |
| turnover | decimal | Y | 成交额，单位元（指数成分股合计）；JSON 中为字符串 |
| volume | int64 | Y | 成交量（指数成分股合计） |
...
CALL_RESPONSE=$(curl -fsS -m 60 -X POST "$MCP_BASE_URL" \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_index_candlesticks_batch","arguments":{"symbols":["000300.SH"],"interval_unit":"day","until_ts_millis":1782370800000,"limit":2}}}')
    ...
    async with streamable_http_client(MCP_BASE_URL) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            result = await session.call_tool(
                'ft_index_candlesticks_batch',
                {'symbols': ['000300.SH'], 'interval_unit': 'day', 'until_ts_millis': 1782370800000, 'limit': 2},
            )
            if result.is_error:
...
|------|------|------|------|------|------|------|------|------|
| 000300.SH | 4900.3838 | 4953.2321 | 4892.2676 | 4943.0197 | 1782284400000 | 1782264600000 | 964945849978.7000 | 31224902500 |
| 000300.SH | 4950.9753 | 5031.9994 | 4944.8760 | 5020.1038 | 1782370800000 | 1782351000000 | 1110040892256.1000 | 34666428900 |
| ... | ... | ... | ... | ... | ... | ... | ... | ... |
