...
> **MCP 工具**：`ft_index_candlesticks_batch`（category: `指数专题/指数行情`）。返回统一 MCP 输出：`structuredContent.metadata` + `structuredContent.data`；`content[0].text` 为同值 JSON。

- 描述：批量查询多个指数的 K 线，并将嵌套结果展开为带 `symbol` 的逐根记录。
- 提示：所有标的必须是指数；分钟 K 的起止时间最多覆盖 3 个自然日。

## 输入参数

| 名称 | 类型 | 必选 | 描述 |
|------|------|------|------|
| symbols | array[string] | Y | 指数代码列表 |
...
> MCP 工具名 `ft_index_candlesticks_batch`。MCP Streamable HTTP 要求先 `initialize` 获取 `Mcp-Session-Id`，再发送 `notifications/initialized`，最后调用 `tools/call`。
...
```json
{"symbol":"000300.SH","open":"4950.9753","high":"5031.9994","low":"4944.8760","close":"5020.1038","ts_millis":1782370800000,"ts_millis_open":1782351000000,"turnover":"1110040892256.1000","volume":34666428900}
```
