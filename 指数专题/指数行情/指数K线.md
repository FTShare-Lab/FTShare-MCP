...
> **MCP 工具**：`ft_index_candlesticks`（category: `指数专题/指数行情`）。返回统一 MCP 输出：`structuredContent.metadata` + `structuredContent.data`；`content[0].text` 是同值的序列化 JSON，不额外返回 Markdown。输入参数 / 输出参数 / 数据样例见下文。
> 文中 `Response`、`items`、`records`、`code`、`message` 等名称仅为字段说明；MCP 对外固定为上述 `metadata/data`。

- 描述：获取单只指数的历史 K 线，包含开盘点位、最高点位、最低点位、收盘点位、成交量和成交额，支持分钟、日、周、月、年周期以及前复权、后复权；仅支持指数标的。
- 数据范围：1991 年至今，不同指数的起始日期可能不同
...
- 提示：
  ...
  - `symbol` 必须是指数代码，格式为 `{代码}.{市场}`；沪市支持 `.XSHG`/`.SH`，深市支持 `.XSHE`/`.SZ`。
  - 不带市场后缀、后缀无法识别或传入非指数标的时，当前外部接口返回系统错误。
...
| 名称 | 类型 | 必选 | 描述 |
|------|------|------|------|
| symbol | string | Y | 指数代码，如 `000300.XSHG`、`399001.XSHE`；也接受 `.SH`、`.SZ` 短后缀 |
    ...
    async with streamable_http_client(MCP_BASE_URL) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            result = await session.call_tool(
                ...
                {'symbol': '000300.SH', 'interval_unit': 'day', 'until_ts_millis': 1782370800000, 'limit': 2},
            )
            if result.is_error:
...
|------|------|------|------|------|------|------|------|
| 4900.3838 | 4953.2321 | 4892.2676 | 4943.0197 | 1782284400000 | 1782264600000 | 964945849978.7000 | 31224902500 |
| 4950.9753 | 5031.9994 | 4944.8760 | 5020.1038 | 1782370800000 | 1782351000000 | 1110040892256.1000 | 34666428900 |
| ... | ... | ... | ... | ... | ... | ... | ... |
