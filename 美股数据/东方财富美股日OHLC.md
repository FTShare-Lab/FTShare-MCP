...
curl -sS -m 10 -X POST <MCP_BASE_URL> \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_eastmoney_us_stock_daily_kline","arguments":{"stock_code": "600519.SH"}}}'
```
...
async def main():
    async with streamablehttp_client('<MCP_BASE_URL>') as (r, w, _):
        async with ClientSession(r, w) as s:
            await s.initialize()
            res = await s.call_tool('ft_eastmoney_us_stock_daily_kline', {"stock_code": "600519.SH"})
