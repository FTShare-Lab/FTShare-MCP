...
- 描述：获取东方财富美股历史日 K 线，包含开盘价、最高价、最低价、收盘价、成交量、成交额和振幅；股票代码必填，可按日期范围筛选，同时指定起止日期时跨度不超过 3 天。
- 数据范围：K 线时间序列
- 单次限量：分页返回，默认 `page=1`、`page_size` 默认 50；区间跨度上限 3 天
- 提示：
...
CALL_RESPONSE=$(curl -fsS -m 60 -X POST "$MCP_BASE_URL" \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_eastmoney_us_stock_daily_kline","arguments":{"stock_code":"AAPL","page":1,"page_size":2,"start_date":"20260624","end_date":"20260624"}}}')
    ...
    async with streamable_http_client(MCP_BASE_URL) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            result = await session.call_tool(
                ...
                {'stock_code': 'AAPL', 'page': 1, 'page_size': 2, 'start_date': '20260624', 'end_date': '20260624'},
            )
            if result.is_error:
...
|------|------|------|------|------|------|
| null | null | 295.355 | 299.7 | 292.94 | 293.08 |
| ... | ... | ... | ... | ... | ... |
