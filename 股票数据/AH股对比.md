...
> **MCP 工具**：`ft_get_stk_ah_comparison`（category: `股票数据/特色数据`）。返回 **markdown 表格文本**（非结构化 JSON）。输入参数 / 输出参数 / 数据样例对照 [ftshare-doc](../../ftshare-doc/api-doc) 原接口。

- 描述：查询 AH 股比价分页数据，返回 `PaginatedResponse<StkAhComparisonItem>`。支持按 `hk_code`（港股）/`ts_code`（A 股）+ 单日或日期区间过滤，返回每对 A/H 标的的历史比价记录：含双方名称/收盘价/涨跌幅、A/H 比价（= A 股收盘价 / 港股收盘价）、A/H 溢价率（=（A/H − 1）× 100）。默认 `page=1`、`page_size=50`，校验层 `page_size` 上限 500，store 层再钳到 1000。
- 数据范围：以服务端返回为准
...
> MCP 工具名 `ft_get_stk_ah_comparison`。MCP Streamable HTTP 要求**先 initialize 拿 `Mcp-Session-Id`，再 `tools/call`**，后续请求带该 header。返回 **markdown 表格文本**。
...
curl -sS -m 10 -X POST <MCP_BASE_URL> \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_get_stk_ah_comparison","arguments":{}}}'
```
...
async def main():
    async with streamablehttp_client('<MCP_BASE_URL>') as (r, w, _):
        async with ClientSession(r, w) as s:
            await s.initialize()
            res = await s.call_tool('ft_get_stk_ah_comparison', {})
