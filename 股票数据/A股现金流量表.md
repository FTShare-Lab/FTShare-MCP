...
> **MCP 工具**：`ft_cashflow`（category: `股票数据/财务数据`）。返回 **markdown 表格文本**（非结构化 JSON）。输入参数 / 输出参数 / 数据样例对照 [ftshare-doc](../../ftshare-doc/api-doc) 原接口。

- 描述：查询 A 股上市公司现金流量表数据，底表 `ashare_cashflow`。返回**宽表**结构（每行一个报告期 + 多个财务科目列），数值字段为 `Option<Decimal>`，优先取「合并调整」报表类型，没有则取「合并未调整」。同一路径支持两种模式：模式A 传 `stock_code` 查单票所有报告期；模式B 不传 `stock_code` 时必填 `year + report_type + page + page_size`，按单个报告期分页查所有票。报告类型 `report_type` 取值 `q1` / `q2` / `q3` / `annual`。
- 数据范围：以服务端返回为准
...
> MCP 工具名 `ft_cashflow`。MCP Streamable HTTP 要求**先 initialize 拿 `Mcp-Session-Id`，再 `tools/call`**，后续请求带该 header。返回 **markdown 表格文本**。
...
curl -sS -m 10 -X POST <MCP_BASE_URL> \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_cashflow","arguments":{}}}'
```
...
async def main():
    async with streamablehttp_client('<MCP_BASE_URL>') as (r, w, _):
        async with ClientSession(r, w) as s:
            await s.initialize()
            res = await s.call_tool('ft_cashflow', {})
...
> 公网 finance/cashflow 返回系统错误，待后端修复后补真实样例
