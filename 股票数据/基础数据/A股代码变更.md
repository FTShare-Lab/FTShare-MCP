...
> **MCP 工具**：`ft_get_stk_code_change`（category: `股票数据/基础数据`）。返回统一 MCP 输出：`structuredContent.metadata` + `structuredContent.data`；`content[0].text` 为同值 JSON。

- 描述：查询一只或多只 A 股的代码变更历史，返回各代码的使用起止区间。
...
| 字段 | 类型 | 描述 |
|------|------|------|
| trade_code | string | 当前证券代码 |
| code | string | 历史代码 |
| name | string | 证券名称 |
| start_date | string | 代码启用日期，`YYYYMMDD` |
| end_date | string/null | 代码停用日期；仍在使用时为 `null` |

## 调用方法（MCP）

> MCP 工具名 `ft_get_stk_code_change`。MCP Streamable HTTP 要求先 `initialize` 获取 `Mcp-Session-Id`，再发送 `notifications/initialized`，最后调用 `tools/call`。
...
```json
{"trade_code":"001872.SZ","code":"1872","name":"招商港口","start_date":"20181226","end_date":null}
```
