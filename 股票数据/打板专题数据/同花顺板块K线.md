...
- 描述：获取单个同花顺板块的历史日 K 线，包括开盘价、最高价、最低价、收盘价和成交量；较早板块的历史数据可追溯至 2007 年。
- 数据范围：2007-08-01 至今，不同板块类型的起始日期可能不同
- 单次限量：无硬性上限，由 page/page_size 分页，默认返回全部行（不传分页参数）
- 提示：
  - `board_code` 必填；csrc 模块板块无 K 线，请求会返回 400。
...
CALL_RESPONSE=$(curl -fsS -m 60 -X POST "$MCP_BASE_URL" \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_ths_board_kline","arguments":{"board_code":"886056","page":1,"page_size":2}}}')
    ...
    async with streamable_http_client(MCP_BASE_URL) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            result = await session.call_tool(
                'ft_ths_board_kline',
                {'board_code': '886056', 'page': 1, 'page_size': 2},
            )
            if result.is_error:
...
| 886056 | 阿尔茨海默概念 | concept | 2023-09-20 | 976.249 | 991.568 | 975.323 | 979.1949999999999 | 256607070 |
| ... | ... | ... | ... | ... | ... | ... | ... | ... |
