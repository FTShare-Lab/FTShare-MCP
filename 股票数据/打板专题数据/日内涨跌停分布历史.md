...
> **MCP 工具**：`ft_daec_distribution_history`（category: `股票数据/打板专题数据`）。返回统一 MCP 输出：`structuredContent.metadata` + `structuredContent.data`；`content[0].text` 是同值的序列化 JSON，不额外返回 Markdown。输入参数 / 输出参数 / 数据样例见下文。
> 文中 `Response`、`items`、`records`、`code`、`message` 等名称仅为字段说明；MCP 对外固定为上述 `metadata/data`。

- 描述：查询当日涨跌停股票数量随时间变化的快照，每分钟一个采样点，包括各时刻的涨停和跌停股票数量。
- 数据范围：当日盘中（每分钟一个快照点）
- 单次限量：返回当日全量采样点，无分页
- 提示：
