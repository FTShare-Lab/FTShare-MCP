...
- 描述：查询龙虎榜异动成交总览，按上交所、深交所和北交所提供上榜标的的代码、名称与涨跌幅，不含营业部买卖明细和成交额。传入 `date` 时查询指定交易日，不传时查询最新数据。
- 数据范围：按交易日更新
...
| 名称 | 类型 | 默认显示 | 描述 |
|------|------|---------|------|
| bjse | array | Y | 北交所上榜标的列表 |
| xshe | array | Y | 深交所上榜标的列表 |
| xshg | array | Y | 上交所上榜标的列表 |
...
不传 `date` 返回最新快照，按交易所分桶（bjse 6 条、xshe 37 条、xshg 23 条）。各市场取前 3 条示例如下。
