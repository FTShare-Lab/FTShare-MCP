...
> **MCP 工具**：`ft_stock_candlesticks_batch`（category: `股票数据/行情数据`）。返回 **markdown 表格文本**（非结构化 JSON）。输入参数 / 输出参数 / 数据样例对照 [ftshare-doc](../../ftshare-doc/api-doc) 原接口。

- 描述：批量获取多个标的历史 K 线（股票/ETF/指数/债券）。返回数组，每项为 `(symbol, Vec<StockCandlestick>)`。支持分/日/周/月/年周期与前/后复权，单标的返回结构与 `stock-candlesticks` 一致。
- 数据范围：以服务端返回为准（参考单标的日K：1991 至今）
- 单次限量：since/until 时间跨度 ≤3 天，无分页，由 limit 控制每标的返回条数
- 提示：
  - `symbols`、`interval_unit`、`until_ts_millis` 必填。
  - since/until 是毫秒时间戳，单标的跨度硬限制 ≤3 天，超过会被拒。
  - 需要长周期分时数据请分段循环调用，或对每个标的并发单接口请求。
...
> MCP 工具名 `ft_stock_candlesticks_batch`。MCP Streamable HTTP 要求**先 initialize 拿 `Mcp-Session-Id`，再 `tools/call`**，后续请求带该 header。返回 **markdown 表格文本**。
...
curl -sS -m 10 -X POST <MCP_BASE_URL> \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_stock_candlesticks_batch","arguments":{"symbols": ["600584.SH"], "interval_unit": "day", "until_ts_millis": 1782800280000}}}'
```
...
async def main():
    async with streamablehttp_client('<MCP_BASE_URL>') as (r, w, _):
        async with ClientSession(r, w) as s:
            await s.initialize()
            res = await s.call_tool('ft_stock_candlesticks_batch', {"symbols": ["600584.SH"], "interval_unit": "day", "until_ts_millis": 1782800280000})
