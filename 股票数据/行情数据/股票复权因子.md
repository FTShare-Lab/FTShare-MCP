...
- 描述：查询股票当日复权因子及截至当日的累计复权因子。指定 `trade_date` 可查询单日快照，未指定 `symbol` 时查询所有标的；指定 `start_date` 和 `end_date` 可查询日期区间内每个交易日的复权因子，区间查询必须指定 `symbol`。
- 数据范围：每标的当前主要返回最新交易日快照
- 单次限量：通过 offset/limit 截断；默认 page/page_size 默认 50
- 提示：
  - 未指定 `symbol` 时仅支持单日扫描；区间扫描必须指定 `symbol`。
  - `trade_date` 为空时默认当天，非交易日回退到前一交易日。
  - `symbol` 输出统一为 `symbol.suffix` 格式（如 `600000.SH`）。
  - 返回分页结构，列表字段为 `records`。
...
CALL_RESPONSE=$(curl -fsS -m 60 -X POST "$MCP_BASE_URL" \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_stock_adjust_factor","arguments":{"symbol":"600519.SH","start_date":"20260623","end_date":"20260624","limit":2}}}')
    ...
    async with streamable_http_client(MCP_BASE_URL) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            result = await session.call_tool(
                ...
                {'symbol': '600519.SH', 'start_date': '20260623', 'end_date': '20260624', 'limit': 2},
            )
            if result.is_error:
...
|------|------|------|------|
| 600519.SH | 20260623 | 1.0 | 8.6771454751862 |
| ... | ... | ... | ... |
