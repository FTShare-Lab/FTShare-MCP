...
- 描述：查询 A 股上市公司现金流量表。每条数据对应一个报告期，优先采用「合并调整」报表，没有时采用「合并未调整」报表。支持按 `stock_code` 查询单只股票全部报告期，或按 `year` 和 `report_type` 查询全市场指定报告期；`report_type` 支持 `q1`、`q2`、`q3` 和 `annual`。
- 数据范围：按报告期查询
...
CALL_RESPONSE=$(curl -fsS -m 60 -X POST "$MCP_BASE_URL" \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_cashflow","arguments":{"stock_code":"600519.SH"}}}')
