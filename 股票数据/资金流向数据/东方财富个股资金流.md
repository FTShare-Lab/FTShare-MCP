...
- 描述：获取东方财富个股日资金流数据，包括主力、超大单、大单、中单和小单净流入及净占比，以及收盘价与涨跌幅。支持按股票代码、交易日或日期区间筛选。数据来源：东方财富。
- 数据范围：资金流类，最新一期或当日快照
...
CALL_RESPONSE=$(curl -fsS -m 60 -X POST "$MCP_BASE_URL" \
  ...
  -H "Mcp-Session-Id: $SID" \
  -H "MCP-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ft_get_eastmoney_stock_flow","arguments":{"symbol":"600522","page":1,"page_size":5}}}')
    ...
    async with streamable_http_client(MCP_BASE_URL) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            result = await session.call_tool(
                ...
                {'symbol': '600522', 'page': 1, 'page_size': 5},
            )
            if result.is_error:
